...
<?php
	...
	WHERE us.st_ativo = 1 && st_report = 1
	...
	if ($result=$mysqli->query($sql)) {
			while ($obj = $result ->fetch_object()) {
				...
				<td>".$obj->ds_report."</td>
				<td>".$obj->dt_report."</td>
				<td><a href='suspender.php?codigo=$obj->cd_reportperfil?&cdbani=$obj->id_reportado' class='btn btn-danger'>Banir por 15 dias</a></td>";
...
<?php
	...
	INNER JOIN usuario AS us ON us.cd_usuario = rs.id_reportados 
	WHERE us.st_ativo = 1 && st_reports = 1 ORDER BY dt_reports DESC";//ele usa o nome, pq nao ta setado, da erro
	if ($result=$mysqli->query($sql)) {
			while ($obj = $result ->fetch_object()) {
				echo "
				<tr class='tr'>
				...
				<td><a href='suspenders.php?cdban=$obj->id_reportados&codig=$obj->cd_reportservico' class='btn btn-danger'>Banir por 15 dias</a></td>";
